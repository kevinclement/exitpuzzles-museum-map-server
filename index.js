...
logger.log('mapjs: Started ExitPuzzles Map server.');
...
fb.db.ref('.info/connected').on('value', function(connectedSnap) {
  if (connectedSnap.val() === true) {
    logger.log('mapjs: firebase connected.');
  } else {
    // dont print an error while its still connecting on first start
    if (_connecting) {
      _connecting = false;
    } else {
      logger.log('mapjs: firebase dropped connection!');
    }
  }
});
...
fb.db.ref('museum/status/mapjs').update({
